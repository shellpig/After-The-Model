...
| 8-C~8-H | ⬜ 規格已完成 / 待實作 | 便利商店 vertical slice + 買賣系統：街道便利商店改為可進入的獨立場景；首演主角正職「AI 善後」關卡（修一台「讀了被辭退店員日記、連其拒絕工作一起繼承」的店控機器人）；蒐證 + 診斷對話（誤導選項 / 全對解鎖拾遺）；店籍主機三段重置（直接 / 先錄殘響）；通關解鎖店內 ShopPanel 買賣（固定價 + 有限庫存 + 可賣折價）+ 街道迷你飲料商店（多商店資料化）。規格 / 契約 / 測試清單已寫入（`遊戲規格書.md` / `開發設計方針.md` / `測試指南.md` / `subdocs/地點/便利商店.md`）；查閱地圖行範圍待校正 |
...
> 行號以 2026-06-10 版為準；大幅改寫後需校正。Phase 8 規格 / 契約 / 測試清單已寫入，尚未實作（⬜ 待開工）。
...
| 8-A 場景骨架 + 轉場 | 2280–2286・2353 表 | 1713–1733 | 624–631 |
| 8-B 前導對話 + 發現錯誤旗標 | 2313–2323・2353 表 | 1734–1766 | 632–638 |
| 8-C 電腦兩段 gate + 接案 | 2287–2312・2353 表 | 1767–1819 | 639–646 |
| 8-D 蒐證 + 診斷對話樹 | 2324–2331・2353 表 | 1820–1836 | 647–655 |
| 8-E 店籍主機三段 + 兩種結局 | 2332–2352・2353 表 | 1837–1893 | 656–668 |
| 8-F 買賣系統核心 + ShopPanel | 2259–2279・2353 表 | 1894–1948 | 669–685 |
| 8-G 迷你飲料商店 + 多商店 | 2353 表 | 1949–1954 | 686–691 |
| 8-H 回歸 + 存讀檔 + GUI 走查 | 2353 表 | 1955–1958 | 692–699 |
| 旗標 / 狀態一覽 | 2370–2383 | — | — |
...
**Phase 8 — 便利商店 vertical slice + 買賣系統：規格 / 契約 / 測試清單已完成、待實作。** 範圍見 Phase 進度表 8-A~8-H 列與 `subdocs/地點/便利商店.md`：可進入的便利商店室內場景、AI 善後修復關卡（蒐證 + 診斷對話 + 店籍主機三段重置）、第一個經濟系統（ShopPanel 買賣、固定價 + 有限庫存 + 可賣折價、多商店資料化）。建議下一步從 8-A（場景骨架 + 轉場）開工。查閱地圖的 Phase 8 行範圍對照待規格定稿後補。
