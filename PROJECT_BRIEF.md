...
| 20 | 📐 規格可實作 | **Act 2D 七號可選 + 和平線 Branch D**：三份文件已寫入實作規格（2026-06-25）；不新增場景 / 系統，沿用 Phase 16 七號對話樹 + Phase 18 回執 + Phase 11 Trust/Trace。canonical：`seven_peace_branch_d` / `seven_betrayal_pending` / `childcare_supply_receipt` / `peace_line_locked`；成功漏斗＝持回執 + 事件前主動 + 不威脅 / 不嘲諷 / 不逼問 + 交還，成功移除回執、Trust↑、Trace 不增或微降；賣回執永久鎖死 D。契約見 `開發設計方針.md > Act 2D 七號可選 + 和平線 Branch D（Phase 20，實作契約）`；驗收見 `遊戲規格書.md > Phase 20`；清單見 `測試指南.md > Phase 20` |
| 21 | 📐 規格可實作 | **Act 3 夜總會 + 林霏核心殘響 + 主題歌整合**（原 Phase 21 主題歌 + 原 Phase 22 夜總會 / 林霏殘響合併；**Phase 22 編號保留不再使用**，23~31 不變）：三份文件已寫入實作規格（2026-06-25）。新增 1 場景 `nightclub` + 1 跨幕殘響 `echo_linfei`；**唯一系統擴充＝EchoDB/EchoPoint 媒體分段門檻**（半收集→女聲歌、全收集→照片），其餘純複用 Phase 9 拾遺 / Phase 10 ducking·Ambient / Phase 14 碎片。canonical：`echo_linfei`（分段媒體 + 可賣）/ `mem_frag_hideout` / `sold_linfei_echo`（賣林霏殘響→結局僅台詞微調的 forward-契約）。結構先行（21-A~C 無素材依賴）、素材後掛（21-D/E）。**外部 blocker（user 提供、不暫代）：夜總會美術 + BGM / 林霏女聲歌 / 林霏殘響照片 /《雨還沒停》母題。** 契約見 `開發設計方針.md > Act 3 夜總會 + 林霏核心殘響 + 主題歌（Phase 21，實作契約）`；驗收見 `遊戲規格書.md > Phase 21`；清單見 `測試指南.md > Phase 21` |
...
依賴：17（殘響系統）、14（碎片旗標）、9（`EchoPoint`/`EchoDB`/`echo_progress`/採集/收購）、11-C（賣→Trace↓）、10（ducking / Ambient bus）。新 canonical：`echo_linfei`（分段媒體 + 可賣）、`mem_frag_hideout`、`sold_linfei_echo`（**forward-契約**：Phase 26~28 結局讀此改少數台詞）。**外部 blocker（user 提供，不暫代）**：夜總會背景美術 + 場景 BGM、林霏女聲主題歌、林霏殘響照片、《雨還沒停》母題音訊。
