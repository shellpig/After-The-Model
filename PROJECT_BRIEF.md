...
**Phase 9 — 拾遺系統（殘響蒐集 + 收藏家）：9-A~9-G 已全數完成。** 主軸：把主角副業「拾遺者」落地為真系統——時鐘取「老舊探測模組」→ 收藏家鹿其琛鑑定 → 解碼手套升級「拾遺手套」→ 感知採集 4 條殘響（7 點跨 5 場景）→ 筆記殘響分頁（`????` 佔位 + 集滿解鎖照片 / 錄音）→ 賣 vs 留（對話式收購、賣後標記已售出且媒體永久失效、鹿家殘響排除且不收）。新場景 `collector_shop` 經街道右端暫時通道進入。規格 / 契約 / 測試清單 / 兩份 subdocs 已寫入（對照表見上方 Phase 9 子階段）。只剩 9-H 全系統回歸與走查。
