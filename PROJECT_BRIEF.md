...
| 21-A | 🟦 待驗收 | 夜總會場景骨架 + 轉場：新增 3 場景（`nightclub_entrance` 大門口＝Art Bible anchor-04 / `nightclub` 門面廳 / `nightclub_back` 後場殘響房）+ 雙向轉場 + BGM 兩軌（前區 `nightclub-1` / 後場 `nightclub-2`）；進入動線＝月台 `commuter_screen` → `learned_topside_shortcut` → 街道最左端 travel「前往上層區」→ `nightclub_entrance`（commit `f1a6549` + `f47223f`，headless PASS）|
| 21-B | 🟦 待驗收 | 林霏核心殘響 + 分段媒體：跨幕殘響 `echo_linfei` 6 段 + 分段媒體門檻（半收集 3 段→女聲歌、全收集 6 段→照片）+ 向後相容舊式 echo；s1~s6 真佈點（s1 大門口 / s2 月台 / s3 聚落左室 / s4 門面廳 / s5·s6 後場碎片右側折返必經）+ test_runner 跨場景佈點守門測試 |
| 21-C | 🟦 待驗收 | 收束碎片「黑戶藏身方式」：`MemoryFragmentArea` `mem_frag_hideout`（gate＝採滿 `echo_linfei`）|
| 21-D | 🟦 待驗收 | 女聲主題歌掛載：林霏女聲歌掛半收集 slot；夜總會美術 3 圖 + BGM 兩軌 + 女聲歌 / 殘響照片 2026-06-27 全到貨 |
| 21-E | 🟦 待驗收 | 《雨還沒停》環境母題：複用既有 `echo_song_rain_doesnt_stop.mp3`，掛地鐵月台 / 地下道聚落 `ThemeMotifPlayer`（Ambient bus / -24dB）；修補 `duplicate()` 後再設 `.loop` 防污染共享快取殘響回放（commit `7058974`）|
| 21-F | 🟦 待驗收 | 可賣 + 回歸 + 存讀檔：`echo_linfei` 可賣（`sell_echo`→credits 400 + Trace↓ + set `sold_linfei_echo` forward-契約）+ 補收藏家賣出選單缺的 `echo_settlement_erased`(P17) / `echo_ada_reset`(P19) + round-trip；本機 headless 全測 PASS（exit 0）。canonical：`echo_linfei` / `mem_frag_hideout` / `sold_linfei_echo` / `learned_topside_shortcut`。待驗收：GUI / 真機（母題聽感、殘響回放不循環）|
| 22 | — 併入 21 | **已併入 Phase 21（編號保留，不再使用）**；原 22-A~D（夜總會場景 / 林霏核心殘響 / 黑戶碎片 / 回歸）整併進 Phase 21；23~31 編號不變 |
| 23-A | 🟦 待驗收 | 場景接線骨架：Phase 21 門面廳 `back_door` 無條件門改四解 gate retrofit；新增保全 NPC（`Bodyguard` Area2D）+ `bar_bot` 互動 + 工牌 examine + ITEMS_DB key item `nightclub_staff_pass`（與 19-B `old_work_badge` 為不同物件，命名 / 顯示名 / 描述刻意拉開）；不新增場景（commit `3056bda`，headless PASS）|
| 23-B | 🟦 待驗收 | 保全對話樹（賄賂 + 假裝身份）+ `credits` condition：`data/dialogue/nightclub_bodyguard.gd`（賄賂 credits≥500 複用 Phase 8 經濟 / 假裝身份 gate `found_staff_pass`）+ DialogueRunner 補 `credits` condition type（最小 op）+ 三語 locale + staff_pass icon。code review 修補：`nightclub.gd._trigger_interaction()` 補回標準 `dialogue_id` dispatch（原缺→保全對話實機不可觸發）|
| 23-C | 🟦 待驗收 | 引開→潛行（一次性限時組合，2026-06-28 redesign）：① 前置 gate＝先與保全對話（`talked_nightclub_bodyguard`）才解鎖 `bar_bot` 引開；② 引發＝保全 + bar_bot 淡出停用 → 中央情境圖鎖操作 5 秒（複用 `set_monologue_active` 擋熱鍵 + `player.set_physics_process(false)`）→ 淡出後解鎖並開 10 秒倒數空窗；③ 空窗內 E `back_door` 溜過 set `passed_nightclub_security`；④ 倒數歸零未潛入＝全暗 messagebox「保全已處理完畢」(`MSG_NIGHTCLUB_GUARD_RETURNED`) → 保全 + bar_bot 歸位；⑤ **引開永久一次性**：`nightclub_bar_bot_used`（持久、納存讀檔），用過後 bar_bot 只給中性訊息。`_bodyguard_off_post` 仍 transient 不存檔；情境圖 `assets/generated/sprites/nightclub_bar_bot_distraction/situation/`；新訊息 key `MSG_NIGHTCLUB_BAR_BOT_PRE_TALK`/`_USED`/`MSG_NIGHTCLUB_GUARD_RETURNED`（移除舊 `MSG_NIGHTCLUB_BAR_BOT_DISTRACTED`）；headless PASS（含一次性 + 視窗到期歸位測試）|
| 23-D | 🟦 待驗收 | 回歸 + 存讀檔 + GUI / 觸控走查：本機 headless 全測 PASS（exit 0）；待 GUI / 純觸控走查（四解各自可玩、保全淡出 / 歸位、刷閘導向側門）。canonical：`passed_nightclub_security` / `found_staff_pass` / `nightclub_staff_pass`。前提硬規則：保全人類不可打死 / 格式化；大門口閘門無 Trace；四解皆不動 Trace |
