...
| 23-C | 🟦 待驗收 | 引開→潛行 transient（兩步組合）：引開（`bar_bot` 故障→保全淡出離崗）+ 潛行（離崗空窗 E `back_door` 溜過）；transient `_bodyguard_off_post` 不存檔，僅 `passed_nightclub_security` 持久；四解任一成功 set `passed_nightclub_security`（commit `238a9e3`）|
