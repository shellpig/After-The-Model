...
> **主線《雨還沒停》v2.3 後續規劃（Phase 11–31，順敘版）**：完整 Phase / 子階段排程已寫入 `遊戲規格書.md > Phase 11+`、`開發設計方針.md > Phase 11+`、`測試指南.md > Phase 11+`（敘事事實來源 `subdocs/主線/雨還沒停v2.3.md`）。**地基 Phase 11（已實作）/ 12 / 13 規格已展開到可實作（📐，見下「Phase 12–13 子階段（三份對照）」行範圍）；Phase 14（Act 1 主線鉤子）三份文件亦已展開到可實作（📐，實作契約見 `開發設計方針.md > Act 1 主線鉤子（Phase 14，實作契約）`）**；**Phase 15 起為 ⬜ 待規劃**；嚴格順敘、不跳號。Phase 12 跳躍架構已拍板（2026-06-19）＝**方案 A 腳本化拋物弧**（不引入全域重力），可直接開工。
