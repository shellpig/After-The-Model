"""Single-frame color-alignment test for the attack sprite.

Reuses the combat_walk 'stylematch v3' pipeline verbatim so the tone matches
by construction: magenta HSV chroma-key -> keep-largest blob -> black outline
-> 3D color transfer + warm-cloth red trim toward the jump/walk palette.

Usage: python _process_one.py <src_png> <dst_png>
"""
import sys
import numpy as np
from PIL import Image, ImageFilter, ImageChops

# --- tunables (copied from combat_walk/_process.py) ---
H_LOW, H_HIGH, S_MIN, V_MIN = 200, 232, 60, 25
# attack video art has heavier baked-in ink than the combat_walk concept art,
# so the added outline is trimmed 4 -> 1 to match combat_walk's outer-edge weight.
OUTLINE_PX = 1
...
im = style_grade(im)
im.save(dst)
# also a magenta-backed preview for quick visual check
bg = Image.new("RGBA", im.size, (255, 0, 255, 255))
Image.alpha_composite(bg, im).convert("RGB").save(dst.replace(".png", "-on-magenta.png"))
