# PROJECT_BRIEF 歸檔 — Phase 進度詳表 + 各階段查閱地圖

> 2026-07-04 自 `PROJECT_BRIEF.md` 搬出歸檔。本檔**不再維護**：
>
> - 一行版進度索引見 `PROJECT_BRIEF.md > Phase 進度`。
> - 新 Phase 的細節寫在三份規格文件（遊戲規格書 / 開發設計方針 / 測試指南）對應 Phase 段落與 git log，不再寫進本檔。
> - 下方「各階段查閱地圖」中的行號以 2026-05~06 版文件為準，**早已漂移，僅供考古**；查段落一律改用標題 grep（規則見 `AGENTS.md > 按 Phase 查閱規則`）。
> - 歸檔時 Phase 28 各列仍寫「程式未開工」，實際 28-A~D 已於 2026-07-03~04 實作完成（commits `880517e` / `87e3246` / `edd1cb7` / `cb77a06` / `4faa523`，headless 435 PASS）；最新狀態以 `PROJECT_BRIEF.md` 一行版為準。
...
| 27-C | ✅ 完成 | 上傳前清洗閘五拍（**清洗第一次硬兌現**，11-D 移入）：掛廣播站上傳終端 `dialogue_id="broadcast_upload"`；①掃描（`echo_count` 兩檔，門檻 9／17 段）→②中性警告（`seven_stopped_partial` Branch B 變體）→③手藝反諷三選（動手清洗／原樣保留／退開）→④代價當場兌現兩版（不告知刪了哪些）→⑤最終確認鎖點寫 `expose_upload_cleaned`＋`expose_upload_done`（中性收尾，不分岔）；退開（任一層）不留半套狀態、下次重走整段；level `broadcast_station.gd` 新增 `expose_upload_done` gate（已上傳→中性 idle examine，不重開樹）。系統擴充唯一＝DialogueRunner `echo_count` condition（讀新只讀 API `GameState.get_collected_echo_segment_count()`）。新增 `data/dialogue/broadcast_upload.gd` + `dialogue_db.gd` 註冊 + `dialogue.csv`/`story.csv` 三語；headless PASS（`tests/manual/test_runner.gd` Phase 27-C 區塊，本機待跑——此 session 無 Godot 執行環境，待使用者於 Windows 端跑 headless 驗證）|
| 27-D | ✅ 完成 | 回歸 + 存讀檔：6 旗標 + `broadcast_station` 場景 round-trip（互斥性、清洗二態、存檔提示皆持久化、讀檔不重播進場 MessageBox、上傳終端 idle gate 持久化）；三選前留檔（`stood_before_own_backup`＋`ending_save_hint_seen`）同一份存檔重讀三次分別走 Reclaim / Protect / Expose 皆可達（含存檔提示不重播）；回歸掃過 25 travel / 門禁、戰鬥③、27 廣播站新掛點、24 追逐 / 攤牌、聚落往返皆不退化；**本 Phase 不碰 Trace / 不做 A/B/C 判定（29）/ 不做結局演出（28）**。headless PASS（`tests/manual/test_runner.gd` Phase 27-D 區塊，本機待跑——此 session 無 Godot 執行環境，待使用者於 Windows 端跑 headless 驗證）|
| 28-A | 📐 規格可實作 | Reclaim 三站腳本化序列（**2026-07-03 規格定案**）：backup_core 灌回演出（2-G 開場鏡像多頁＋`trace` 兩檔壓垮拍）→ `apartment_entrance` 晚訣別（`affinity_wan` 兩檔）＋情境圖 CG 拍（走進雨裡）**永久消失** → 公寓痕跡拍＋set `ending_reclaim_played` 靜止停（30-A 掛點）；站間自動 travel、全程禁存。程式未開工 |
...
> 行號以 2026-07-03 Phase 28 規格寫入後重校版為準；大幅改寫後需校正。Phase 27 = 地下廣播站 + 三結局路由 + 清洗閘兌現；**✅ 27-A~D 全部已完成**（本機 headless 待跑，見各子階段）。**1 新場景**（`broadcast_station`，Expose 唯一出口、單向、可存檔、無 NPC）、**1 系統小擴充**（DialogueRunner `echo_count` condition，23 補 `credits` 同例）、**零新存讀檔 schema**（5 旗標全走 story_flags）。A/B/C 判定與 Trace 臨界值＝Phase 29；結局演出＝Phase 28/29；**本 Phase 不碰 Trace**。
