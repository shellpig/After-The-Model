...
func _ready() -> void:
	...
	await get_tree().process_frame
	if UIMode.get_mode() != UIMode.Mode.NONE:
		printerr("FAIL 28-B: three-beat fixed farewell should close back to UIMode.NONE after three confirms, got mode=", UIMode.get_mode())
		get_tree().quit(1)
		return
	if not is_instance_valid(wan_npc_phase28b) or wan_npc_phase28b.is_queued_for_deletion():
		printerr("FAIL 28-B: Protect farewell must NOT remove NpcWan (晚不消失，留在原地) — Reclaim/Protect asymmetry!")
		get_tree().quit(1)
		return
	if main_inst_phase28b.get_current_scene_id() != "apartment" or main_inst_phase28b.get_current_entry_point_id() != "epilogue_home":
		...
		get_tree().quit(1)
		return
	print("PASS 28-B: three-beat fixed farewell plays through, Wan stays (does not disappear), and travels to apartment:epilogue_home.")
	...
	settlement_inst_phase28c.scene_transition_requested.connect(func(scene_id, entry_point_id, _payload): wu_travel_phase28c = {"scene_id": scene_id, "entry_point_id": entry_point_id})
