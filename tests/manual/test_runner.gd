...
func _ready() -> void:
	...
	GameState.collect_echo_segment("echo_clerk", "s1")
	# Use the existing mp3 for audio path dynamically so it loads cleanly during testing
	db_clerk["audio_path"] = "res://assets/audio/echoes/echo_song_rain_doesnt_stop.mp3"
