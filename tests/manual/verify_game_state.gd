...
func test_change_item_id_api() -> void:
	print("Running Test 10: change_item_id API Verification...")
	inventory_signal_count = 0

	# Clear backpack and seed worn_rubiks_cube
	GameState.inventory.clear()
	for i in range(GameState.inventory_slots):
		GameState.inventory.append({})

	var add_ok = GameState.add_item("worn_rubiks_cube", 1)
	assert(add_ok == true, "Should add worn_rubiks_cube")

	var inv = GameState.get_inventory()
	var cube_instance_id = inv[0].get("instance_id")
	assert(inv[0].get("item_id") == "worn_rubiks_cube", "Should be worn_rubiks_cube")

	# Change item ID
	var change_ok = GameState.change_item_id(cube_instance_id, "decoder_cube")
	assert(change_ok == true, "Should mutate item ID successfully")
	assert(inventory_signal_count == 2, "Should emit inventory_changed when changing ID")
